    ...
    return (
        <div className="p-3 flex items-center justify-between bg-slate-900 bg-gradient-to-r from-slate-900 to to-gray-800 pl-14">
            ...
            <div className="flex items-center space-x-5 pr-7" >
                <input type="text" placeholder="Search movie" className="text-black p-2 w-64 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm"
                onChange={ (e) => setSearch(e.target.value) } value={search}></input> 
                <button className="bg-red-600 text-white px-4 py-2 rounded-lg hover:text-gray-800 transition duration-300" onClick={ () => onSearch(search) } >Search</button>
            </div>
        </div>
    )
}
